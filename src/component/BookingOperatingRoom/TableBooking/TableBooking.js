...
import { CustomerService } from '../TableBooking/CustomerService';
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            filters: {
                ...
                'room': { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
            ...
            <div className="datatable-doc-demo">
                <div className="card">
                    <DataTable value={this.state.customers} paginator className="p-datatable-customers" header={header} rows={10}
                        ...
                        globalFilterFields={['room', 'user.name', 'dateStart', 'timeStart', 'dateTo', 'timeTo']} emptyMessage="No customers found."
                        currentPageReportTemplate="Showing {first} to {last} of {totalRecords} entries">
                        <Column field="room" header="ห้องผ่าตัด" sortable style={{ minWidth: '14rem' }} />
                        <Column field="user.name" header="ผู้จองใช้งาน" sortable filterField="country.name" style={{ minWidth: '14rem' }} 
                            body={this.countryBodyTemplate} />
                        <Column field="dateStart" header="เริ่มวันที่" sortable filterField="date" dataType="date" style={{ minWidth: '8rem' }} />
                        <Column field="timeStart" header="เวลา" sortable filterField="date" dataType="date" style={{ minWidth: '8rem' }} />
                        <Column field="dateTo" header="ถึงวันที่" sortable filterField="date" dataType="date" style={{ minWidth: '8rem' }} />
                        <Column field="timeTo" header="เวลา" sortable filterField="date" dataType="date" style={{ minWidth: '8rem' }} />
                    </DataTable>
                </div>
            </div>
        );
